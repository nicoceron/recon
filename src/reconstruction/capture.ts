...
export async function captureReconstruction(
  ...
  const viewports = opts.viewports?.length ? opts.viewports : DEFAULT_RECONSTRUCTION_VIEWPORTS;
  const pages: PageReconstructionEvidence[] = [];
  const urlList = Array.from(urls);
  const attemptedPages = urlList.length;

  const capturePage = async (url: string): Promise<PageReconstructionEvidence | undefined> => {
    ...
    const viewportResults = await Promise.all(viewports.map(async (viewport) => {
      ...
      try {
        ...
        if (viewport.name === 'desktop') {
          await captureInteractionStates(page, extracted.viewport.interactions);
        }
        ...
        logger.error({ url, viewport: viewport.name, err: (err as Error).message }, 'reconstruction-viewport-capture-failed');
        return undefined;
      } finally {
  ...
  let cursor = 0;
  const workerCount = Math.min(2, urlList.length);
  ...
  return { capturedAt: new Date().toISOString(), viewports, pages };
}
...
async function captureInteractionStates(page: Page, interactions: InteractionEvidence[]): Promise<void> {
  ...
  for (const interaction of interactions.slice(0, 24)) {
    if (interaction.domIndex < 0) continue;
    const locator = page.locator('body *').nth(interaction.domIndex);
...
): Promise<Array<{ label: string; scrollY: number; path: string; screenshot: Buffer }>> {
  const captures: Array<{ label: string; scrollY: number; path: string; screenshot: Buffer }> = [];
  try {
